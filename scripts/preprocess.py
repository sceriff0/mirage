...
# BaSiC imports (optional, guarded for test/dev environments).
try:
...
# Local imports (guarded fallback for save_h5).
try:
...
# Logging config (guarded fallback for test/dev environments).
try:
...
def apply_basic_correction(
    ...
    **basic_kwargs
    ...
    # Split into FOVs.
    fov_stack, positions, _ = split_image_into_fovs(image, fov_size, overlap=0)

    # Initialize BaSiC model.
    if BaSiC is None:
        raise ImportError("BaSiC is not available. Please install basicpy.")
