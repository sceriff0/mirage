"""Shim: ``tiled_manifest`` now lives in the ``stare`` package as ``stare.manifest``.
...
(``from tiled_manifest import ...`` after a ``sys.path.insert(0, .../bin/utils)``) keeps
resolving. It replaces itself in ``sys.modules`` with the package module rather
than re-exporting names, so ``import tiled_manifest`` yields the SAME module object the
stages use -- a test that monkeypatches an attribute on it patches the real one.
"""

import sys

from stare import manifest as _impl
