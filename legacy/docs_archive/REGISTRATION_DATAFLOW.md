...
### Step 3C.3: Segmentation Overlap Estimation (Legacy)

Segmentation-overlap error estimation has been removed from the active pipeline surface.
The current workflow exposes feature-based registration error estimation through `enable_feature_error` only.
