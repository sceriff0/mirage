...
### Segmentation-Based Parameters (Legacy)
Segmentation-overlap error estimation parameters are no longer part of the active pipeline surface.
...
### Run Feature-Based Error Estimation
```bash
...
The active pipeline surface provides **feature-based registration error estimation**:
...
Segmentation-overlap metrics are documented for historical context only and are not currently user-configurable via pipeline parameters.
